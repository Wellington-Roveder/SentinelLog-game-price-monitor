...
def carregar_dados():
    ...
    df = pd.DataFrame(
        db_dados, columns=["id", "produto", "valor", "data_verificacao", "loja_barata"]
    )
    return df
...
if not df.empty:
    ...
    jogo_selecionado = st.selectbox(
        "Selecione um jogo para ver o historico:", df["produto"].unique()
    )
    df_filtrado = df[df["produto"] == jogo_selecionado]
    df_filtrado["data_verificacao"] = pd.to_datetime(
        df_filtrado["data_verificacao"], dayfirst=True
    )

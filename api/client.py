        ...
        def wrapper(*args, **kwargs):
            ...
            while retries < max_retries:
                try:
                    ...
                    retries += 1
                    logger.warning(
                        f"Tentando novamente, Tentativa: {retries} Motivo {e}"
                    )

                    if (
                        isinstance(e, HTTPError)
                        and e.response.status_code not in status_forcelist
                    ):
                    ...
                    if isinstance(e, HTTPError) and e.response.status_code == 429:
                        ...
                        logger.error(f"429 | Retry-After: {retry_after}")
...
class APIClient:
    ...
    def get(self, endpoint):
        try:
            ...
            )  # reutiliza sessao, timeout para nao dar trava para sempre
